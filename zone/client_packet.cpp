...
void Client::Handle_Connect_OP_ZoneEntry(const EQApplicationPacket *app)
{
	...
	*/
	...
	/* Load Character Data */
	query = fmt::format(
		"SELECT `lfp`, `lfg`, `first_login`, `guild_id`, `rank`, `exp_enabled`, `tribute_enable`, `extra_haste`, `illusion_block`, `ingame` FROM `character_data` LEFT JOIN `guild_members` ON `id` = `char_id` WHERE `id` = {}",
		cid
	);
	auto results = database.QueryDatabase(query);
	for (auto row : results) {
		if (row[3] && Strings::ToInt(row[3]) > 0) {
			guild_id             = Strings::ToInt(row[3]);
			guildrank            = row[4] ? Strings::ToInt(row[4]) : GUILD_RANK_NONE;
			guild_tribute_opt_in = row[6] ? Strings::ToBool(row[6]) : 0;
		}

		SetEXPEnabled(Strings::ToBool(row[5]));
		SetExtraHaste(Strings::ToInt(row[7]), false);
		SetIllusionBlock(Strings::ToBool(row[8]));
		...
		if (row[3]) {
			first_login = Strings::ToUnsignedInt(row[2]);
		}

		if (row[10]) {
			ingame = Strings::ToBool(row[9]);
		}
	}
...
void Client::Handle_OP_AdventureMerchantSell(const EQApplicationPacket *app)
{
	...
	uint32 price = EQ::Clamp(
		price,
		EQ::ClampUpper(
			(item->LDoNPrice + 1) /* * item->LDoNSellBackRate / 100 */, // LDoNSellBackRate is post titanium, need to check this formula for titanium era
			item->LDoNPrice
		),
		item->LDoNPrice
	);
...
void Client::Handle_OP_PopupResponse(const EQApplicationPacket *app)
{
	...
	auto t = GetTarget();
	if (t) {
		parse->EventMob(EVENT_POPUP_RESPONSE, t, this, [&]() { return std::to_string(popup_response->popupid); });
	}
}
